namespace MyPalClara.Memory.Prompts;

/// <summary>
/// mem0-style prompt templates for memory extraction and update decisions.
/// Ported from mem0/configs/prompts.py.
/// </summary>
public static class MemoryPrompts
{
    /// <summary>
    /// Fact extraction prompt — extracts facts from user messages only.
    /// Returns JSON {"facts": [...]}.
    /// </summary>
    public static string UserFactExtractionPrompt => $$"""
        ...
        - Make sure to return the response in the format mentioned in the examples. The response should be JSON with a key as "facts" and corresponding value will be a list of strings.

        Following is a conversation between the user and the assistant. You have to extract the relevant facts and preferences about the user, if any, from the conversation and return them in the JSON format as shown above.
        """;

    private const string UpdateMemoryPromptTemplate = """
        ...
        - **Example**:
            ...
            - New Memory:
                {"memory" : [
                    {"id" : "0", "text" : "Name is John", "event" : "NONE"},
                    {"id" : "1", "text" : "Loves cheese pizza", "event" : "NONE"}
                ]}
        """;

    /// <summary>
    /// Build the complete update memory prompt with existing memories and new facts.
    /// Maps real UUIDs to sequential integers to prevent LLM hallucination.
    /// </summary>
    public static string BuildUpdateMemoryPrompt(
        IReadOnlyList<(string Id, string Text)> existingMemories,
        IReadOnlyList<string> newFacts)
    {
        string currentMemoryPart;
        if (existingMemories.Count > 0)
        {
            var memoryEntries = string.Join(",\n    ",
                existingMemories.Select(m => $$"""{"id" : "{{m.Id}}", "text" : "{{EscapeJson(m.Text)}}"}"""));
        ...
        var factsStr = string.Join(", ", newFacts.Select(f => $"\"{EscapeJson(f)}\""));

        return $$"""
            {{UpdateMemoryPromptTemplate}}
